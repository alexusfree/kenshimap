...
LS.Send = {}
LS.Send.Marker = {}
LS.Send.Popup = L.popup().setContent('<div><input id="sendText" type="text" style="border-color:#a7a7a7;border:solid;border-width:2px;border-radius:5px;height:30px;" size="30" onkeyup="L.LocShare.Send.UpdateMessage( this )" placeholder="Введите сообщение"/></div><div style="height:35px;"><button style="border-style:solid;border-radius:5px;border-color:#3d94f6;float:right;color:white;background-color:#3d94f6;height:35px;font-size:15px;line-height:3px;margin:5px;" onclick="copyPrompt()">получить ссылку</button></div></div>')
LS.Receive = {}
LS.Receive.Marker = {}
LS.Receive.Popup = L.popup()
var sendIcon = L.icon({
//  iconUrl: "js/images/IconMapSend.png",
...
receiveIcon = L.icon({
//  iconUrl: "js/images/IconMapReceive.png",
  iconUrl: "js/images/marker-icon.png",
  iconSize:     [25, 41], // size of the icon
  iconAnchor:   [12, 41], // point of the icon which will correspond to marker's location
  popupAnchor:  [0, -30] // point from which the popup should open relative to the iconAnchor
...
L.Control.ShareLocation = L.Control.extend({
    ...
    onAdd: function () {
        ...
        userIcon.src = 'js/images/IconLocShare.png'
        this.link.href = '#';
...
function copyPrompt() {
  window.prompt("Отправьте это местоположение.", '' + 
